...
data Pro (size :: Size) (context :: Context) where
    ...
    ProList :: List (Pro 'One context) -> Pro size context
...
newtype TagName = TagName Chars
data Attrs = Attrs (Set Flag) (Map FieldName FieldValue)
newtype Flag = Flag Chars
newtype FieldName = FieldName Chars
...
data List (element :: *) where
    ...
    -- | A packed list of characters. See "Data.Text".
